...
deploy_vm() {
  ...
  if [[ "$?" == "0" ]]; then
    ...
    sshpass -p "$PASSWORD" ssh "$USER"@"$VM_netid.$mgip" /bin/bash << EOF &>> /tmp/pve_vm_manager.log && \
      wget -q --save-cookies /home/"$USER"/cookies.txt 'https://docs.google.com/uc?export=download&id=1_4fM9nmWofMOfuF1XC2TP6W0R41u72uH' -O- | sed -rn 's/.*name="uuid" value=\"([0-9A-Za-z_\-]+).*/\1/p' > /home/"$USER"/google_uuid.txt
      wget --load-cookies /home/"$USER"/cookies.txt -O /home/"$USER"/wulin.k1.tar.gz 'https://drive.usercontent.google.com/download?export=download&id=1_4fM9nmWofMOfuF1XC2TP6W0R41u72uH&confirm=t&uuid='$(</home/"$USER"/google_uuid.txt)
      tar -jxvf wulin.k1.tar.gz
      sudo rm -r cookies.txt google_uuid.txt wulin.k1.tar.gz &>> /dev/null
